...
func BenchmarkBst_InOrderInsert(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if found { // small chance zero is input randomly
			duplicates++
		}
	}
...
func TestBST_TraverseIter(t *testing.T) {
	...
	b.traverseIter(func(n *node[int, int]) {
		sum += n.val
	})

	if sum != 114 {
		t.Errorf("total sum of tree is 114, got %d", sum)
	}
}

// recursive traversal is better until trees have > 1million nodes
func BenchmarkBst_TraverseIter(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		// traverse but do nothing
		tree.traverseIter(func(*node[int, int]) {})
	}
}

func BenchmarkBst_TraverseRecursive(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		// traverse but do nothing
		tree.traverse(tree.root, func(*node[int, int]) {})
	}
}

func valueSlice[K any, V any](b *BST[K, V]) []V {
	var x []V
...
func keySlice[K any, V any](b *BST[K, V]) []K {
	var x []K
	b.traverse(b.root, func(n *node[K, V]) {
		x = append(x, n.key)
	})
