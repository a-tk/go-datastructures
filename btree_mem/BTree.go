...
func New[K any, V any](degree int, compare func(K, K) int) (b *BTree[K, V]) {
	return &BTree[K, V]{
		degree:  degree,
...
func (b *BTree[K, V]) Search(k K) (val V, found bool) {
	return b.search(b.root, k)
}

func (b *BTree[K, V]) search(x *node[K, V], k K) (val V, found bool) {
	i := 0
	for i < x.n && b.compare(x.keys[i].key, k) < 1 {
		i++
	}
	if i < x.n && b.compare(x.keys[i].key, k) == 0 {
		return x.keys[i].val, true
	} else if x.leaf {
		return val, false
	} else {
...
func (b *BTree[K, V]) Insert(k K, v V) (old V, replaced bool) {
	r := b.root
	if r.n == 2*b.degree-1 {
		s := b.splitRoot()
...
func (b *BTree[K, V]) insertNonFull(x *node[K, V], k K, v V) (old V, replaced bool) {
	...
	if i != -1 {
		//duplicate
		previous := x.keys[i].val
		x.keys[i] = container[K, V]{key: k, val: v}
		...
		x.keys[i+1] = container[K, V]{key: k, val: v}
		b.size++
		x.n = x.n + 1
		return old, false
	} else {
		...
		if x.children[i].n == 2*b.degree-1 {
			...
			if b.compare(x.keys[i].key, k) < 0 {
				...
				previous := x.keys[i].val                   // save previous value
				x.keys[i] = container[K, V]{key: k, val: v} // replace it with the new value
				return previous, true
			}
		}
